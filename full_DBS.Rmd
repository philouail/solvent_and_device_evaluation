...
png(paste0("paper_figures/Figure2C1.png"), width = 5, height = 8,
...
barplot(colSums(num_features),
    col = leg_solvent,
    ylab = "Number of features", space = 0.05, ylim = c(0, 3500))
...
# Plot
png(paste0("paper_figures/Figure2C2.png"), width = 8, height = 8,
    units = "cm", res = 2000, pointsize = 6, type = "cairo-png")
...
grid(lwd = 0.5)
legend("topleft", legend = names(intensity_solvent), col = leg_solvent, pch = 16, 
       cex = 0.8, horiz = TRUE, bty = "n", text.width = 2.3)
...
ylim_intensity <- range(unlist(intensity_solvent))
plot(intensity_solvent[[1]], type = "b", pch = 16, xlab = "",
     ylab = "Log 2 intensity", col = leg_solvent[1], xaxt = "n", ylim = ylim_intensity)
for (i in 2:length(intensity_solvent)) {
  lines(intensity_solvent[[i]], type = "b", pch = 16, col = leg_solvent[i])
}
axis(1, at = 1:length(intensity_solvent[[1]]), 
...
# Plot the UpSet plot
library(UpSetR)
png(paste0("paper_figures/Figure2C3.png"), width = 15, height = 10,
    units = "cm", res = 2000, pointsize = 6, type = "cairo-png")
upset(as.data.frame(upset_df), sets = c("MetOH_H2O", "ACN", "MetOH", "ACN_MetOH"),
      sets.bar.color = leg_solvent, mainbar.y.label = "Number of common features",
      keep.order = TRUE, mainbar.y.max= 2000)
dev.off()
```
...
load("Rdata_files/DBS/SumExp_full_normalisation.RData")
res_mh <- res_full[, res_full$solvent == "MEOH_H2O"]
res_m <- res_full[, res_full$solvent == "MEOH"]
res_a <- res_full[, res_full$solvent == "ACN"]
res_am <- res_full[, res_full$solvent == "ACN_MEOH"]

nb_fts_mh <-  colSums(!is.na(assay(res_mh, "raw")))
nb_fts_m <-  colSums(!is.na(assay(res_m, "raw")))
nb_fts_a <-  colSums(!is.na(assay(res_a, "raw")))
nb_fts_am <-  colSums(!is.na(assay(res_am, "raw")))

mean_cp <- cbind(
    MetOH_H2O = mean(nb_fts_mh),
    MetOH = mean(nb_fts_m),
    ACN = mean(nb_fts_a),
    ACN_MetOH = mean(nb_fts_am)
) 

mean_cp

sd_cp <- cbind(
    MetOH_H2O = sd(nb_fts_mh),
    MetOH = sd(nb_fts_m),
    ACN = sd(nb_fts_a),
    ACN_MetOH = sd(nb_fts_am)
)

sd_cp

sum_nb <- rbind(mean_cp, sd_cp)
rownames(sum_nb) <- c("Mean", "Standard deviation")
sum_nb <- t(sum_nb)

cbind("Solvent group" = row.names(sum_nb), sum_nb)

sum_nb <- as.data.frame(sum_nb)

test <- cbind("Solvent group" = row.names(sum_nb), sum_nb)

write.xlsx(test, "table_DBS.xlsx", rowNames = TRUE)
# Install required package

library(gt)
pretty_table <- gt(test) %>%
  tab_header(
    title = "Whatman: mean and standard deviation of the number of features per solvent",
  ) %>%
  fmt_number(
...
pretty_table
gtsave(pretty_table, "Whatman_table.png")
```
...
write.csv(toberefined, "toberefined_DBS_solvent.csv")
```
...
I now have a refined list of compounds that I can use to plot the results. 
...
# save feature matrix 
df <- res_full[idx,]
df <- cbind( match_c, assay(df, "norm"))
write.csv(assay(df), "features_annotated_DBS.csv")
```
...
#General plot - now without flagged features
png(paste0("paper_figures/Annot_DBS_summary.png"), width = 5, height = 8,
    units = "cm", res = 2000, pointsize = 6, type = "cairo-png")
layout(mat = matrix(1:3, ncol = 1), height = c(0.3, 0.3, 0.8))
par(mar = c(1, 4.5, 1, 3))
barplot(colSums(num_features),
...
# Plot number of features
png(paste0("paper_figures/Annot_DBS_rtslices.png"), width = 8, height = 8,
    units = "cm", res = 2000, pointsize = 6, type = "cairo-png")
ylim_features <- c(0, max(unlist(num_features_solvent)))
plot(num_features_solvent[,1], col = leg_solvent[1], ylab = "Number of features",
     xlab = "", type = "b", pch = 16, xaxt = "n", ylim = ylim_features,
...
```{r eval=FALSE, fig.height=9, fig.width=8, include=FALSE}
ftc <-function(res_solvent, fts_idx) {
    tmp <- rowMedians(assay(res_solvent, "norm"), na.rm = TRUE)
    cuts_tmp <- cuts[fts_idx]
    t <- split(tmp, cuts_tmp) |> 
...
# Plot the UpSet plot
library(UpSetR)
png(paste0("paper_figures/annot_DBS_upset.png"), width = 15, height = 10,
    units = "cm", res = 2000, pointsize = 6, type = "cairo-png")
upset(as.data.frame(upset_df), sets = c("MetOH_H2O", "ACN", "MetOH", "ACN_MetOH"),
      sets.bar.color = leg_solvent, mainbar.y.label = "Number of common features", 
      keep.order = TRUE, mainbar.y.max= 100)
dev.off()
```
