...
png(paste0("paper_figures/Figure2A1.png"), width = 5, height = 8,
...
# Plot
png(paste0("paper_figures/Figure2A2.png"), width = 8, height = 8,
    units = "cm", res = 2000, pointsize = 6, type = "cairo-png")
...
# Plot the UpSet plot
library(UpSetR)
png(paste0("paper_figures/Figure2A3.png"), width = 15, height = 10,
    units = "cm", res = 2000, pointsize = 6, type = "cairo-png")
upset(as.data.frame(upset_df), sets = c("MetOH_H2O", "ACN", "MetOH", 
                                        "ACN_MetOH"),
      sets.bar.color = leg_solvent, 
      mainbar.y.label = "Number of common features", keep.order = TRUE, 
      mainbar.y.max= 3000)
dev.off()
```
...
load("Rdata_files/CP/SumExp_full_normalisation.RData")
...
write.xlsx(test, "table_CP.xlsx", rowNames = TRUE)
...
pretty_table <- gt(test) %>%
  tab_header(
    title = "Capitainer: mean and standard deviation of the number of features per solvent",
  ) %>%
  fmt_number(
...
pretty_table
gtsave(pretty_table, "CP_table.png")
```
...
write.csv(assay(df), "features_annotated_CP.csv")
```
...
#General plot - now without flagged features
png(paste0("paper_figures/Annot_CAP_summary.png"), width = 5, height = 8,
    units = "cm", res = 2000, pointsize = 6, type = "cairo-png")
layout(mat = matrix(1:3, ncol = 1), height = c(0.3, 0.3, 0.8))
par(mar = c(1, 4.5, 1, 3))
barplot(colSums(num_features),
...
# Plot number of features
png(paste0("paper_figures/Annot_CAP_rtslices.png"), width = 8, height = 8,
    units = "cm", res = 2000, pointsize = 6, type = "cairo-png")
ylim_features <- c(0, max(unlist(num_features_solvent)))
plot(num_features_solvent[,1], col = leg_solvent[1], ylab = "Number of features",
     xlab = "", type = "b", pch = 16, xaxt = "n", ylim = ylim_features,
...
# Plot the UpSet plot
library(UpSetR)
png(paste0("paper_figures/annot_CAP_upset.png"), width = 15, height = 10,
    units = "cm", res = 2000, pointsize = 6, type = "cairo-png")
