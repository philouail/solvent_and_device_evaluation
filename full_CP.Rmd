...
I separate the dataset by solvent
This markdown is for the Capitainer device, and
...
leg_solvent <- brewer.pal(8, name = "Dark2")[c(6, 5, 3, 7)]
...
idx_mh  <- res_full$solvent == "MetOH_H2O" & res_full$sample_role != "Blank"
idx_m  <- res_full$solvent == "MetOH" & res_full$sample_role != "Blank"
idx_a  <- res_full$solvent == "ACN" & res_full$sample_role != "Blank"
idx_am <- res_full$solvent == "ACN_MetOH" & res_full$sample_role != "Blank"
...
```{r}
fts_def <- featureDefinitions(full)[, c("MetOH_H2O", "ACN", "MetOH", "ACN_MetOH", 
...
MetOH <- fts_def$MetOH >= 2 & fts_def$possible_contaminants == TRUE
MetOH_H2O <- fts_def$MetOH_H2O >= 2 & fts_def$possible_contaminants == TRUE
ACN <- fts_def$ACN >= 2 & fts_def$possible_contaminants == TRUE
ACN_MetOH <- fts_def$ACN_MetOH >= 2 & fts_def$possible_contaminants == TRUE

# Summary compare amount of cont. between solvent
list_cont <- c(sum(MetOH_H2O), sum(ACN), sum(MetOH), sum(ACN_MetOH))
...
MetOH_H2O <- featureDefinitions(full)$MetOH_H2O >= 2
ACN <- featureDefinitions(full)$ACN >= 2
MetOH <- featureDefinitions(full)$MetOH >= 2
ACN_MetOH <- featureDefinitions(full)$ACN_MetOH >= 2

num_features <- c(sum(MetOH_H2O), sum(ACN), sum(MetOH), sum(ACN_MetOH))
...
f <- factor(names(noise), levels = names(leg_solvent))
group <- split(noise, f)
...
fts_solvents <- list(MetOH_H2O = MetOH_H2O,
                 MetOH = MetOH,
                 ACN = ACN,
                 ACN_MetOH = ACN_MetOH)
...
intensity_solvent <- list(
    MetOH_H20 = ftc(idx_mh, MetOH_H2O),
    ACN = ftc(idx_a, ACN),
    MetOH = ftc(idx_m, MetOH),
    ACN_MetOH = ftc(idx_am, ACN_MetOH)
    )
...
# Plot the UpSet plot
library(UpSetR)
upset(data.frame(upset_df), sets = c("MetOH_H2O", "ACN", "MetOH", "ACN_MetOH"), sets.bar.color = leg_solvent)
```
...
fts_def <- featureDefinitions(full)[md_full$feature_id, 
                                    c("MetOH_H2O", "ACN", "MetOH", "ACN_MetOH", 
                                      "possible_contaminants")]
...
MetOH <- unique_fts$MetOH >= 2 & unique_fts$possible_contaminants == TRUE
MetOH_H2O <- unique_fts$MetOH_H2O >= 2 & unique_fts$possible_contaminants == TRUE
ACN <- unique_fts$ACN >= 2 & unique_fts$possible_contaminants == TRUE
ACN_MetOH <- unique_fts$ACN_MetOH >= 2 & unique_fts$possible_contaminants == TRUE
...
MetOH <- unique_fts$MetOH >= 2 & unique_fts$possible_contaminants == FALSE
MetOH_H2O <- unique_fts$MetOH_H2O >= 2 &
    unique_fts$possible_contaminants == FALSE
ACN <- unique_fts$ACN >= 2 & unique_fts$possible_contaminants == FALSE
ACN_MetOH <- unique_fts$ACN_MetOH >= 2 & 
    unique_fts$possible_contaminants == FALSE

# Define the number of annotated features for each solvent
num_compounds <- c(sum(MetOH_H2O), sum(ACN), sum(MetOH), sum(ACN_MetOH))
...
idx_mh <- sampleData(full)$solvent == "MetOH_H2O"
idx_m <- sampleData(full)$solvent == "MetOH"
idx_a <- sampleData(full)$solvent == "ACN"
idx_am <- sampleData(full)$solvent == "ACN_MetOH"

#need annotated features names for each solvent 
fts_solvents <- list(MetOH_H2O = unique(md_full$feature_id[MetOH_H2O]),
                 MetOH = unique(md_full$feature_id[MetOH]),
                 ACN = unique(md_full$feature_id[ACN]),
                 ACN_MetOH = unique(md_full$feature_id[ACN_MetOH]))
...
intensity_solvent <- list(
    MetOH_H20 = ftc(idx_mh, fts_solvents$MetOH_H2O),
    ACN = ftc(idx_a, fts_solvents$ACN),
    MetOH = ftc(idx_m, fts_solvents$MetOH),
    ACN_MetOH = ftc(idx_am, fts_solvents$ACN_MetOH)
    )
...
# Create a data frame for the UpSet plot
upset_df <- data.frame(
  MetOH_H2O = as.integer(MetOH_H2O),
  ACN = as.integer(ACN),
  MetOH = as.integer(MetOH),
  ACN_MetOH = as.integer(ACN_MetOH)
)

# Plot the UpSet plot
upset(upset_df, sets = c("MetOH_H2O", "ACN", "MetOH", "ACN_MetOH"), 
