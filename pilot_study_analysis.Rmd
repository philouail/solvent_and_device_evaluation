...
MZML_PATH <- "pilot_study_files/mzmL files/"
...
p <- pheatmap(sim_matrix, annotation_col = ann,
         annotation_colors = list(devices = leg_device), show_rownames = F, 
         show_colnames = F)

p

png(paste0("paper_figures/FigureS4.png"), width = 15, height = 10,
    units = "cm", res = 2000, pointsize = 5, type = "cairo-png")
  grid::grid.newpage()
...
chromatogram(full[idx_QC], aggregationFun = "max", msLevel = 1, chunkSize = 3) |>
    plot(main = "BPC QC", col = leg_qc, lwd = 1)

# all sample 
chromatogram(full, aggregationFun = "max", msLevel = 1, chunkSize = 3) |>
...
eics <- chromatogram(
    ...
    mz = as.matrix(is[, c("mzmin", "mzmax")]), msLevel = 1, chunkSize = 3)
...
full <- findChromPeaks(full, param = param, chunkSize = 3L)
...
full <- refineChromPeaks(full, param = param, chunkSize = 3)
...
We subsequently perform a subset-based alignment.
...
#lets also plot all of them before and after alignment
...
param <- PeakGroupsParam(span = 0.5,
                         ...
                         subsetAdjust = "average")
full <- adjustRtime(full, param = param, chunkSize = 3L)
...
full <- fillChromPeaks(full, param = ChromPeakAreaParam(), chunkSize = 3)
...
full <- filterFeatures(full, BlankFlag(blankIndex = idx_blank, qcIndex = idx_QC, threshold = 2))
...
na_idx <- apply(assay(res_QC, "raw_filled"), MARGIN = 1, function(x) all(is.na(x)))
res_QC <- res_QC[!na_idx, ]

full <- full[!sampleData(full)$sample_type %in% c("blank", "QC"),
             keepFeatures = TRUE]
res_full <- res_full[, !res_full$sample_type %in% c("blank", "QC")]

#remove features that only have NA in the study samples
na_idx <- apply(assay(res_full, "raw_filled"), MARGIN = 1, function(x) all(is.na(x)))
res_full <- res_full[!na_idx, ]
```
...
```{r echo=TRUE, fig.height=5, fig.width=10}
tmp <- apply(assay(res_QC, "raw_filled"), MARGIN = 1, na_unidis)
assays(res_QC)$raw_filled_imputed <- t(tmp)
...
png(paste0("paper_figures/FigureS2_2.png"), width = 5, height = 8,
    units = "cm", res = 2000, pointsize = 6, type = "cairo-png")
par(mfrow = c(2, 1), mar = c(1, 4, 3, 1))
barplot(apply(is.na(combine_df),MARGIN = 2, sum), col = c("blue", "lightblue"), 
        ylab = "Number of missing values", xaxt = "n", main = "QC1 vs QC2")
boxplot(log2(combine_df), col = c("blue", "lightblue"), 
...
```{r echo=TRUE, fig.height=6, fig.width=5}
png(paste0("paper_figures/FigureS2_1.png"), width = 6, height = 7,
    units = "cm", res = 2000, pointsize = 6, type = "cairo-png")
par(mar = c(4, 4, 3, 2))
boxplot(log2(assay(res_QC, "raw_filled")),
        ylab = expression(log[2]~intensity~filled~data), xaxt = "n",
...
# boxplot(log2(assay(res_QC, "raw_filled")), xaxt = "n",
...
# Plot
png(paste0("paper_figures/Figure3_1.png"), width = 5, height = 8,
    units = "cm", res = 2000, pointsize = 6, type = "cairo-png")
...
# Plot
png(paste0("paper_figures/Figure3_2.png"), width = 8, height = 8,
    units = "cm", res = 2000, pointsize = 6, type = "cairo-png")
...
# Plot the UpSet plot
library(UpSetR)
png(paste0("paper_figures/Figure3_3.png"), width = 15, height = 10,
    units = "cm", res = 2000, pointsize = 6, type = "cairo-png")
...
# Average feature number per sample per devices 

```{r}
#load sumexp - to be removed later 
load("SumExp_full_preprocessing.RData")

res_clean <- res_full[, !res_full$sample_type %in% c("QC", "blank")]

result_table <- c()

for (device in unique(res_clean$device)) {
    res <- res_clean[, res_clean$device == device]
    for (sample in unique(res$sample)) {
        res_s <- res[, res$sample == sample]
        mean_nb_fts <- mean(colSums(!is.na(assay(res_s, "raw_filled"))))
        sd_nb_fts <- sd(colSums(!is.na(assay(res_s, "raw_filled"))))
        result_table <- rbind(result_table, cbind(device, sample, mean_nb_fts, sd_nb_fts))
    }
}

colnames(result_table) <- c("Device", "Sample ID", "Mean", "Standard deviation")

result_table <- as.data.frame(result_table)

#order by sample number 
result_table <- result_table[order(result_table$`Sample number`),]

# Install required package

library(gt)
pretty_table <- gt(result_table) %>%
  tab_header(
    title = "Devices comparison: mean and standard deviation of the number of features per solvent",
  ) %>%
  fmt_number(
...
write.xlsx(result_table, "feature_number_per_sample_per_device.xlsx", rowNames = FALSE)

pretty_table
gtsave(pretty_table, "Devices_table.png")

barplot(as.numeric(result_table$Mean), col = leg_device,
        names.arg = result_table$`Sample ID`,
        ylab = "Mean number of features", xlab = "Sample ID", 
        main = "Mean number of features per sample per devices")
```
...
I know have a refined list of compounds that I can use to plot the results. 
...
I will use the function `mclosest` to find the closest match between the
compounds and the features. 

```{r}
query <- cbind(compounds$Average_Mz, compounds$Average_Rtmin)
target <- cbind(rowData(res_full)$mzmed, rowData(res_full)$rtmed)

idx <- mclosest(query, target, ppm = c(50, 0), tolerance = c(0,10))

nrow(query)
length(unique(idx))
match_c <- compounds[!is.na(idx),]

idx <- idx[!is.na(idx)]
# save feature matrix 
df <- res_full[idx,]
df <- cbind( match_c, assay(df, "raw_filled"))
write.csv(assay(df), "features_annotated_pilot_study.csv")
```
...
# Plot
png(paste0("paper_figures/Annot_pilot_study_summary.png"), width = 5, height = 8,
    units = "cm", res = 2000, pointsize = 6, type = "cairo-png")
layout(mat = matrix(1:3, ncol = 1), height = c(0.3, 0.3, 0.8))
par(mar = c(1, 4.5, 1, 3))
barplot(colSums(num_features), col = leg_device, ylab = "Number of features", 
...
# Plot
png(paste0("paper_figures/Annot_pilot_study_rtslices.png"), width = 8, height = 8,
    units = "cm", res = 2000, pointsize = 6, type = "cairo-png")
layout(mat = matrix(1:2, ncol = 1), heights = c(0.5, 0.5))
par(mar = c(0.5, 4.5, 2, 3))
...
axis(1, at = 1:length(num_features_solvent[,1]), labels = FALSE)
grid(lwd = 0.5)
legend("topright", legend = names(intensity_solvent), col = leg_device, pch = 16, 
       cex = 1, horiz = TRUE, bty = "n")
...
# Plot the UpSet plot
library(UpSetR)
png(paste0("paper_figures/annot_pilot_study_upset.png"), width = 15, height = 10,
    units = "cm", res = 2000, pointsize = 6, type = "cairo-png")
upset(as.data.frame(upset_df), sets = c("Whatman", "Capitainer", "Mitra", "Plasma"),
      sets.bar.color = leg_device,  mainbar.y.label = "Number of common features", 
